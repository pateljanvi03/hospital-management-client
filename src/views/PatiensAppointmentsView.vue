      ...
      <div>
        <div class="flex flex-col">
          <modal
            ...
            :reset="true"
          >
            <PatientAppointmentForm :editableData ="editableData" :hospitals="hospitals" :patients="patients" :doctors="doctors" @refresh ="loadData"
            ></PatientAppointmentForm>
          </modal>
          <div>
            <div class="sm:flex justify-between">
              <h1 class="text-2xl font-semibold text-gray-900">Patients' Appointment</h1>
              <button
                type="button"
                 @click="showForm"
                  ...
                  >
                    <table class="min-w-full divide-y divide-gray-300">
                      ...
                      <tbody class="divide-y divide-gray-200 bg-white">
                        ...
                        <tr v-else v-for="patientAppointment in patientsAppointments" :key="patientAppointment._id">
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6"
                          >{{ patientAppointment.patient.name }}</td>
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6"
                          >{{ patientAppointment.hospital.name }}</td>
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6"
                          >{{ patientAppointment.doctor.name }}</td>
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6"
                          >{{ patientAppointment.startTime}}</td>
                          <td
                            class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6"
                          >{{ patientAppointment.endTime }}</td>
                          <td
                          ...
                          <td>
                            <a
                              ...
                              @click="deleteData(patientAppointment)"
                            >Delete</a>
                          </td>
                        </tr>
                      </tbody>
...
export default {
  ...
  methods: {
    async loadData() {
      this.isLoading = true;
      let response = await axios.get('/patient-appoinment');
      this.patientsAppointments = response.data.patients;
      this.patientsAppointments.map(function(x) {
